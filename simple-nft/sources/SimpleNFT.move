...
module  SNFT::SimpleNFTScripts{
	...
	public entry fun test_mint_with_image(sender: signer){
		let name = b"test nft";
	...
	public entry fun mint_with_image(sender: signer, name: vector<u8>, image_url: vector<u8>, description: vector<u8>){
		let metadata = NFT::new_meta_with_image(name, image_url, description);
		let nft = SimpleNFT::mint(&sender,metadata);
		NFTGallery::deposit(&sender, nft);
	}

	public entry fun test_mint_with_image_data(sender: signer){
		let name = b"test nft";
		let description = b"test description";
	...
	public entry fun mint_with_image_data(sender: signer, name: vector<u8>, image_data: vector<u8>, description: vector<u8>){
